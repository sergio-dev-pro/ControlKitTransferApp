...
function KitsDrawerScreen({ navigation }) {
  ...
  const handleQRCodeCamisa = async (ticketCode) => {
    ...
    console.log('showQrCodeCamisa', showQrCodeCamisa);
...
const DeliveryByCPF = ({ onCancelDeliveryByCPF, mustSelectShirtSize }) => {
  ...
  const handleQRCodeCamisa = async (ticketCode) => {
    ...
    // if (!showQrCodeCamisa) return; REMOVIDO PARA PERMITIR CONFIRMAÇÃO VIA MODAL
          ...
          renderItem={({ item: ticketsUser, index }) => (
            <View style={{ flexDirection: 'row', alignItems: 'center' }} key={ticketsUser.id}>
              ...
              <Text h5 style={{ fontSize: 15, paddingRight: 4, flex: 1 }}>
                {[ticketsUser.ticketProviderExternalId || '', ticketsUser.sector || '', ticketsUser.day || '', ticketsUser.kitDeliveredAt ? "ENTREGUE" : null]
                  .filter(Boolean)
                  .join(' - ')}
              </Text>
            </View>
