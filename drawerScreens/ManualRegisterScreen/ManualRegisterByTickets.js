...
const continueRegistryTickets = () => {
  if (step === 2) {
    ...
    if (birthDate.length === 10) {
      ...
      // 1. Crie o objeto da data de nascimento em UTC
      const dateObj = new Date(Date.UTC(year, month - 1, day));

      // 2. Verifique se os componentes da data são válidos (evita datas como 30/02/2023)
      const isValidDate =
        dateObj.getUTCDate() === day &&
        dateObj.getUTCMonth() + 1 === month &&
        dateObj.getUTCFullYear() === year;

      // 3. Obtenha a data de HOJE e zere as horas para uma comparação justa
      const today = new Date();
      today.setUTCHours(0, 0, 0, 0); // Compara apenas a data, não a hora

      // 4. Verificação final combinada
      const isReasonableYear = year >= 1900; // Apenas um limite inferior
      const isFutureDate = dateObj > today;   // A nova verificação crucial
    ...
    if (!address.street || !address.number || !address.city || !address.state || !address.zipcode) {
      Alert.alert('', 'Preencha todos os campos obrigatórios do endereço.');
      return;
    }
